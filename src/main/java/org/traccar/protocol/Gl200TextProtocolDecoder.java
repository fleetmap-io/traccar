...
public class Gl200TextProtocolDecoder extends BaseProtocolDecoder {
    ...
    private void decodeStatus(Position position, long value) {
        ...
        if (BitUtil.check(ignition, 4) || BitUtil.check(ignition, 6)) {
            position.set(Position.KEY_IGNITION, false);
    ...
    private Object decodeTtr(Channel channel, SocketAddress remoteAddress, String[] v) throws ParseException {
        ...
        position.set("option1", v[index++]);
        position.set("option2", v[index++]);
        position.set("option3", v[index++]);
        position.set("option4", v[index++]);
        position.set("option5", v[index]);
