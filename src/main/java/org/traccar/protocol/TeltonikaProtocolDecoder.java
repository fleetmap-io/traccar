...
public class TeltonikaProtocolDecoder extends BaseProtocolDecoder {
    ...
    private void decodeOtherParameter(Position position, int id, ByteBuf buf, int length) {
        ...
        switch (id) {
            ...
            case 37:
            case 81:
                long canSpeed = readValue(buf, length, false);
                if (canSpeed > 0) {
                    position.set("gpsSpeed", position.getSpeed());
                    position.setSpeed(canSpeed / 1.852);
                }
                break;
            case 82:
            ...
            case 390:
            case 84:
                long level = readValue(buf, length, false);
                if (level > 0) {
            ...
            case 85:
                ...
                break;
            case 89:
                long levelPerc = readValue(buf, length, false);
                position.set(Position.PREFIX_IO + id, levelPerc);
                position.set(Position.KEY_FUEL_LEVEL, levelPerc);
                break;
            case 90:
            ...
            case 10806:
            case 10807:
                position.set("humidity" + (id-10803), readValue(buf, length, false));
                break;
            default:
                position.set(Position.PREFIX_IO + id, readValue(buf, length, false));
                break;
